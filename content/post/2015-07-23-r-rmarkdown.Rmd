...
age_group <- function(x){
    ...
    if(x<35) {return("Under 30")}
    else if (x<50) {return("35 to 50")}
    else if (x <70) {return("50 to 70")}
    else {return("70+")}
    }
    else{return(NA)}
}
...
ggplotly(forbes_data_industry %>% 
  ...
  facet_wrap(.~variable, scales = "free"), tooltip =  c("Industry","value") )
```
...
ggplotly(forbes_data_age %>% 
  ggplot(aes(x = reorder(Age_group,value, max))) +
   geom_bar(aes(y = value, fill = Age_group), stat = "Identity") +
   geom_text(aes(y = value, label = value),position = position_stack(vjust = 0.5), color = "#ffffff", size = 3, check_overlap = TRUE) +
     coord_flip() +
  theme_minimal() +
  theme(legend.position = "none") + 
  labs(x = "", y = "Count / Wealth(Trillion USD)")+
  facet_wrap(.~variable, scales = "free"), tooltip =  c("Age group","value") )
```
